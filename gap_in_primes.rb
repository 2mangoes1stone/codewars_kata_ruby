...
def gap(g, m, n)
  # [*m..n].each do |value|
  #   # is_prime = (2..value/2).none? {|i| value % i == 0}
  #   is_prime = Math.sqrt(value).floor.downto(2).none? {|i| value % i == 0}
  #   if is_prime
  #     primes << value
  #   end
  # end
  ...
  Prime.each(n).select { |p| p >= m }[0..-2].zip(primes[1..-1]).find { |a, b| b - a == g }
